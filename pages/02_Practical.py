...
MAX_CONTEXT_TOKENS = 1024
MAX_CHUNK_SIZE = 1000
RESERVED_TOKENS_FOR_OUTPUT = 128
...
def gpt2_generate_answer(user_prompt, question, context, tokenizer, model, max_new_tokens=200, temperature=0.7):
    full_prompt = f"{user_prompt}\nQuestion: {question}\nContext: {context}\nAnswer:"
    inputs = tokenizer(full_prompt, return_tensors="pt", truncation=True).to(model.device)
    try:
        outputs = model.generate(
...
# Streamlit UI
st.set_page_config(page_title="GPT-2 PDF Q&A with Speech", layout="wide")
st.title("📄 PDF Q&A with GPT-2 and Speech")
...
if st.session_state.get("doc_chunks") and st.session_state.get("doc_embeddings"):
    ...
    default_prompt = (
        ...
        "Answer based on the provided context without fabrication."
    )
    prompt_options = {
        "Helpful Assistant (Default)": default_prompt,
        "Rude & Sarcastic": "You are sarcastic and blunt.",
        "Cheerful Assistant": "You are cheerful and positive.",
        "Monotone Assistant": "You respond in a flat, unemotional tone.",
        "Custom...": None
    }

    with st.expander("🎭 Choose Assistant Personality / Prompt"):
        persona = st.selectbox(
            "Select a persona for the assistant:",
            list(prompt_options.keys()),
            key="persona_select"
        ...
        if not question.strip():
            ...
            if tokenizer is None or gpt2_model is None:
                ...
                with st.spinner("Generating answer..."):
                    ...
                    selected_chunks = [st.session_state["doc_chunks"][hit["corpus_id"]] for hit in hits]
                    context_clean = build_context(selected_chunks, tokenizer, max_tokens=MAX_CONTEXT_TOKENS)
                    answer = gpt2_generate_answer(
                        user_prompt, question, context_clean, tokenizer, gpt2_model,
                        max_new_tokens=int(max_tokens), temperature=temperature
                    )
                    st.session_state["last_answer"] = answer
